...
| 変数名 | 値 | 環境 |
|---|---|---|
| `VITE_AUTH_MODE` | `bound` | Production |
| `VITE_AUTH_HUB_URL` | `https://digicollabo.com` | Production |
...
> **認証モード（bound）について**: このカートは独自ログイン画面を持ちません。認証はハブ（`digicollabo.com`）で行い、URL経由で JWT を受け取ります。トークンなしで `cart.digicollabo.com` に直接アクセスした場合、自動的にハブへリダイレクトされます（詳細は仕様書 `digicollab_sso_architecture_spec_v2.md` §5）。
...
## 6. Supabase Auth リダイレクトURL（bound モードでは追加不要）

このカートは **bound モード**（SSO受信側）で動作するため、Supabase Auth の Redirect URLs は **ハブ側（`digicollabo.com`）のみに絞る**運用になります。

- カートドメイン（`cart.digicollabo.com`）は Redirect URL に登録**しません**
- Magic Link は発行しません
- 認証はすべてハブで完結し、JWT が URL 経由で引き継がれます

> 旧版の「カートドメインを Supabase に許可」手順は**廃止**。ハブ側の Supabase Auth 設定のみを正しく保ってください。
